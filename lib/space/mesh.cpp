...
void Mesh::computeInterfaces() {
  const int q = elem[0]->getBasis()->getQ();
  ...
  for (int e = 0; e < q; ++e) {
    ...
    double u1L = *(LeftElem->getU1(q));
    double u1R = *(RightElem->getU1(0));
    double u2L = *(LeftElem->getU2(q));
    double u2R = *(RightElem->getU2(0));
    double u3L = *(LeftElem->getU3(q));
    double u3R = *(RightElem->getU3(0));

    double f1L = *(LeftElem->getF1(q));
    double f1R = *(RightElem->getF1(0));
    double f2L = *(LeftElem->getF2(q));
    double f2R = *(RightElem->getF2(0));
    double f3L = *(LeftElem->getF3(q));
    ...
    double invWJ_L = (1.0 / w[q]) * *(LeftElem->getInvJ());
    double invWJ_R = (1.0 / w[0]) * *(RightElem->getInvJ());

    /// Add the correction to the elements
    LeftElem->correctDivF1(q, invWJ_L * (f1star - f1L));
    LeftElem->correctDivF2(q, invWJ_L * (f2star - f2L));
    LeftElem->correctDivF3(q, invWJ_L * (f3star - f3L));
...
void Mesh::applyDirichlet() {
  ...
  // --- RIGHT BOUNDARY ---
  int last = n - 1;
  double u1_int_R = *(elem[last]->getU1(q));
  double u2_int_R = *(elem[last]->getU2(q));
  double u3_int_R = *(elem[last]->getU3(q));
  double f1_int_R = *(elem[last]->getF1(q));
  double f2_int_R = *(elem[last]->getF2(q));
  double f3_int_R = *(elem[last]->getF3(q));
  ...
  double invWJ_R = (1.0 / w[q]) * *(elem[last]->getInvJ());
  elem[last]->correctDivF1(q, invWJ_R * (f1s_R - f1_int_R));
  elem[last]->correctDivF2(q, invWJ_R * (f2s_R - f2_int_R));
  elem[last]->correctDivF3(q, invWJ_R * (f3s_R - f3_int_R));
}
