...
void diffuse(elem::Element *elem, double *nodes_eps, const int n) {
  ...
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, invJ, D, n, elem->getU1(), 1, 0.,
              du1, 1);
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, invJ, D, n, elem->getU2(), 1, 0.,
              du2, 1);
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, invJ, D, n, elem->getU3(), 1, 0.,
              du3, 1);
  for (int i = 0; i < n; ++i) {
    du1[i] *= nodes_eps[i];
    du2[i] *= nodes_eps[i];
  ...
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, invJ, D, n, du1, 1, 0., tmp1, 1);
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, invJ, D, n, du2, 1, 0., tmp2, 1);
  cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, invJ, D, n, du3, 1, 0., tmp3, 1);

  // Subtract from divF: diffusion adds to RHS, which means subtracting from divF
  for (int i = 0; i < n; ++i) {
